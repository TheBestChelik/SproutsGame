...
class Delaunator:
    ...
    def __init__(self, vertices: List[Vertex]):
        ...
        triangles = self.create(coordinates, vertices)

    def create(self, coords, vertices: List[Vertex]):
        n = len(coords) >> 1
        ...
        self._ids = [None] * n
        self._dists = [None] * n
        triangles = self.update(coords, vertices)

        return triangles

    def update(self, coords, vertices: List[Vertex]):
        n = len(coords) >> 1
        ...
        for i in range(0, n):
            d = calculate_distance(c, vertices[i])
        ...
        for i in range(0, n):
            if (i == i0):
                continue
            d = calculate_distance(Vertex(i0x, i0y, Color.BLUE), vertices[i])

            if (d < minimum_distance and d > 0):
                i1 = i
                minimum_distance = d
        ...
        for i in range(0, n):
            if (i == i0 or i == i1):
                continue
            r = circumradius(i0x, i0y, i1x, i1y, vertices[i].x, vertices[i].y)
        ...
        if (minimum_radius == math.inf):
            # order collinear points by dx (or dy if all x are identical)
            # and return the list as a hull
            for i in range(0, n):
                self._dists[i] = (
                    vertices[i].x - vertices[0].x) or (vertices[i].y - vertices[0].y)
        ...
        for i in range(0, n):
            self._dists[i] = calculate_distance(
                vertices[i], Vertex(center[0], center[1], Color.BLUE))
        ...
        for k in range(0, len(self._ids)):
            ...
            while True:
                q = self.hullNext[e]
                if orient(x, y, vertices[e].x, vertices[e].y, vertices[q].x, vertices[q].y):
                    break
                e = q
            ...
            t = self._addTriangle(
                e, i, self.hullNext[e], -1, -1, self.hullTri[e])

            # recursively flip triangles from the point until they satisfy the Delaunay condition
            self.hullTri[i] = self._legalize(t + 2, vertices)
            ...
            while True:
                q = self.hullNext[n]
                if not (orient(x, y, vertices[n].x, vertices[n].y, vertices[q].x, vertices[q].y)):
                    break
                t = self._addTriangle(
                    n, i, q, self.hullTri[i], -1, self.hullTri[n])
                self.hullTri[i] = self._legalize(t + 2, vertices)
            ...
            if (e == start):
                while True:
                    q = self.hullPrev[e]
                    if not (orient(x, y, vertices[q].x, vertices[q].y, vertices[e].x, vertices[e].y)):
                        break
                    t = self._addTriangle(
                        q, i, e, -1, self.hullTri[e], self.hullTri[q])
                    self._legalize(t + 2, vertices)
                    self.hullTri[q] = t
            ...
            self.hullHash[self._hashKey(vertices[e].x, vertices[e].y)] = e
    ...
    def _legalize(self, a, vertices: List[Vertex]):
        i = 0
        ar = 0
        ...
        while True:
            ...
            illegal = inCircle(
                vertices[p0].x, vertices[p0].y,
                vertices[pr].x, vertices[pr].y,
                vertices[pl].x, vertices[pl].y,
...
def calculate_distance(vertex1: Vertex, vertex2: Vertex):
