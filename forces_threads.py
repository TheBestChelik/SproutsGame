...
def calculate_forces(vertices, edges, start_index, end_index, optimum_length, vertex_attraction_coefficient, vertex_repulsion_coefficient, edge_repulsion_coefficient, resultant_forces):
...
def calculate_resultant_forces(vertices: Set[Vertex], edges: Set[Tuple[Vertex, Vertex]], boundaries: Set[Tuple[Vertex, Vertex]], optimum_length: float) -> Dict[Vertex, Vector]:
    ...
    ver_list = list(vertices)
    total_vertices = len(ver_list)
    number_of_threads = 4
    segment_size = len(vertices) // number_of_threads
    ...
    for i in range(number_of_threads):
        ...
        end_index = (i + 1) * \
            segment_size if i < number_of_threads - 1 else total_vertices

        thread = threading.Thread(target=calculate_forces, args=(ver_list, edges, start_index, end_index, optimum_length,
                                                                 vertex_attraction_coefficient, vertex_repulsion_coefficient, edge_repulsion_coefficient,
                                                                 resultant_forces))
        thread.start()
